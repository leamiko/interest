  ...
  SexyLightbox = {
    ...
    overlay: {
      ...
      resize: function(x, y) {
        ...
        var chromebugfix = $.support.safari ? (win.x - 25 < document.body.clientWidth ? document.body.clientWidth : win.x) : win.x;
    ...
    initialize: function(options) {
      ...
      this.Background.css({'position':'relative', 'overflow': 'hidden'});
      this.Desc         = $('#'+name+'-Desc');
      this.Desc.css({
          'width': '100%',
      ...
      $(window).bind('scroll', $.bind(this, function() {
        ...
        // Add by Zonkee start
        if (this.options.withWaterfall == 1) {
            this.close();
        }
        // End
      }));
    ...
    hook: function(enlace) {
      enlace = $(enlace);
      enlace.blur();
      this.show((enlace.attr("title") || enlace.attr("name") || ""), enlace.attr("href"), (enlace.attr('rel') || false), "", enlace.attr('name'));
    },
    ...
    */
    ...
    getInfo: function (image, id) {
      image=$(image);
      IEuta = $('<a id="'+this.options.name+'-'+id+'" title="'+image.attr('title')+'" rel="'+image.attr('rel')+'" name="'+image.attr('name')+'">&nbsp;</a>');
    ...
    show: function(caption, url, rel, force, _name) {
      this.showLoading();
      ...
      if ( force=='image' || baseURL.match(imageURL) )
      {
          ...
          this.lightbox.height = params['height']+($.support.opera?2:0);
          this.replaceBox({'resize' : 1});
    ...
    showNavBar: function() {
      if (this.navigator)
      {
        ...
        // Add by Zonkee start
        this.Desc.empty().html('<div style="width:96%;margin: 6px auto;">' + this.lightbox.name + '</div>');
        // End
